...
export default function UserMenu({ onSignOut, role, purpose }: Props) {
  ...
  useEffect(() => {
    // Delay user load to avoid blocking initial render
  ...
  if (loading) {
    return <div className="w-8 h-8 rounded-full bg-[#161b22] border border-[#30363d]" />
  }
          ...
          <div className="px-4 py-2 border-t border-[#21262d] bg-[#0d1117]">
            ...
            <div className="flex flex-wrap gap-1">
              {['Google Sign-in', 'Email Confirm', 'Export PDF'].map(f => (
                <span key={f} className="text-xs px-1.5 py-0.5 bg-[#161b22] border border-[#21262d] rounded text-[#30363d]">
                  {f}
                </span>
              ))}
            </div>
