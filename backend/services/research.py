...
async def run_research(
    ...
    # Generate human-readable report using Haiku
    report["generated_report"] = await generate_report(report, http_client)
...
async def generate_report(
    ...
    """
    Uses Claude Haiku to synthesize raw research data into a
    clean, readable report for the War Room advisors.

    ELI5: The 4 librarians came back with raw notes and numbers.
    This function gives those notes to a writer (Haiku) who turns
    them into a proper research report that the advisors can read
    and cite clearly.

    Why Haiku (not Sonnet):
    This is a summarization task — straightforward, no deep reasoning.
    Haiku is 4x cheaper and fast enough. Save Sonnet for the Chairman.

    Args:
        raw_research: the dict returned by run_research()
        http_client: shared httpx client (not used directly here,
                     kept for consistency)

    Returns:
        A clean markdown research report as a string.
    """
    from services.llm_client import call_fast

    idea = raw_research.get("idea", "unknown idea")
    summary = raw_research.get("summary", {})
    sources = raw_research.get("sources", {})

    # Build the synthesis prompt
    prompt = f"""You are a technical research analyst. 
Synthesize this research data into a clear, structured report.

PROJECT IDEA: {idea}

RAW RESEARCH DATA:
{json.dumps(raw_research, indent=2)[:8000]}

Write a research report with these exact sections:

## Market Signal
How crowded is this space? What does GitHub show?

## Community Interest  
What does the developer community think? (HackerNews data)

## Academic Foundation
How much research exists? Is this cutting edge or mature?

## Ecosystem Health
How well supported is the tech stack? (StackOverflow data)

## Key Opportunities
2-3 specific opportunities based on the data.

## Key Risks
2-3 specific risks based on the data.

## Overall Assessment
One paragraph summary. Be direct and data-driven.
Cite specific numbers from the research.

Keep the report under 500 words. Be specific, not generic."""

    try:
        report_text = await call_fast(
            system="You are a precise technical research analyst. Always cite specific numbers.",
            user=prompt,
            max_tokens=800,
        )
        logger.info("Research report generated by Haiku")
        return report_text

    except Exception as e:
        logger.error(f"Report generation failed: {e}")
        # Fallback: return structured summary without AI
        return f"""## Research Report — {idea}

## Market Signal
GitHub shows {summary.get('similar_repos', 0)} similar repositories.
Market signal: {summary.get('market_signal', 'unknown')}.

## Community Interest
HackerNews shows {summary.get('community_posts', 0)} related discussions.
Community interest: {summary.get('community_interest', 'unknown')}.

## Academic Foundation
arXiv shows {summary.get('academic_papers', 0)} related papers.
Research activity: {summary.get('research_activity', 'unknown')}.

## Ecosystem Health
StackOverflow shows {summary.get('so_questions', 0)} related questions.
Ecosystem health: {summary.get('ecosystem_health', 'unknown')}.

## Overall Assessment
Research completed in {raw_research.get('elapsed_seconds', 0)}s.
Data collected from 4 sources. Report generation failed — using summary data.
"""